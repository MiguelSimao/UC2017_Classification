...
from keras.layers import Input, Dense, GaussianNoise, Dropout, LSTM
...
def create_net(batch_size):
    # Weight initializer
    initer = keras.initializers.RandomNormal(seed=new_seed)
    
    inputs = Input(batch_shape=(batch_size,F_train.shape[1],F_train.shape[2]))
    x = Dense(512, activation='tanh', kernel_initializer=initer)(inputs)
    x = GaussianNoise(.4)(x)
    x = BatchNormalization()(x)
    x = LSTM(256,activation='tanh', dropout=0.,stateful=True,
...
net_predict = create_net(batch_size=1)
...
def ts_evaluate(y,t,mask):
    # takes as input the model prediction y, the target class and the padding mask
