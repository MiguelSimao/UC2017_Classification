...
def create_net(batch_size):
    ...
    x = GaussianNoise(.1)(x)
    x = LSTM(256,activation='tanh',dropout=0.,stateful=True,return_sequences=True,implementation=2)(x)
    x = LSTM(256,activation='tanh',dropout=0.,stateful=True,return_sequences=True,implementation=2)(x)
    x = Dropout(0.5)(x)
...
history = net.fit(x=F_train,y=T_train,
                  sample_weight=mask_train.astype('int'),
                  validation_data=(F_val,T_val,mask_val.astype('int')),
                  epochs=1000,
...
acc_test = ts_evaluate(y_test,T_test,mask_test > 0) * 100
